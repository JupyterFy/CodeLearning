...
'''
【使用的语法糖】：
1. 列表推导式和input()和字符串的split()方法的结合
...
【与官方答案的对比：本人被完爆】：
1. 官方答案中使用了二维数组（列表），这种形式更接近矩阵的定义
2. 在处理数据输入时，也按照了矩阵的形式来输入，界面更直观友好。
3. 在数据量较大时，该种方式在计算矩阵的 Hadamard 乘积时，代码执行效率更高。复杂度n*n

相反，本人下面的处理方式，
1. 仅仅使用了一维数组，并且严重依赖python本身的语法糖！
如果使用C语言，该种方法将不成立。
2. 此外，在计算矩阵的 Hadamard 乘积时，复杂度较高。复杂度n*n*n*n。足足比官方答案大了一倍！
'''
